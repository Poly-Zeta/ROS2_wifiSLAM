...
def generate_launch_description():
    ...
    # ------------------------------------------------------------
    # Launch configs (あとでライン探索しやすいように引数化)
    # ------------------------------------------------------------
    ...
    declare_infra_profile = DeclareLaunchArgument(
        'infra_profile',
        default_value='424x240x30',
        description='RealSense infra profile'
    )
    ...
    #
    # 要件反映:
    # - VO用に infra1 + depth + IMU
    # - 色付き地図用に color + aligned_depth_to_color
    # - infra2 は使わない
    # ------------------------------------------------------------
    ...
    realsense = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            # RTAB-Map側で地図/点群を作るのでRealSenseのPCLは切る
    ...
    #
    # まずは要求どおり IMU を入れるが、
    # 立ち上がり安定性のため wait_imu_to_init は false にしてある。
    # （IMU自体は remap して使う）
    # ------------------------------------------------------------
    visual_odom = Node(
        ...
        parameters=[{
            ...
            # infra1/depth はまず exact sync 優先
            'approx_sync': False,

            # IMUは使うが、初期化待ちで詰まりにくくする
    ...
    #
    # 要件:
    # - 色付き点群のため color + aligned_depth_to_color
    # - /scan 拘束を使う
    # - odom は上の infra-based VO を使う
    #
    # scan は launch arg use_scan で ON/OFF 可能にしてある
    # ------------------------------------------------------------
    rtabmap = Node(
        ...
        parameters=[{
            ...
            # 2D LiDAR で拘束、3DoF運用寄り
            'Reg/Strategy': '1',
            'RGBD/NeighborLinkRefining': 'true',
            'Grid/FromDepth': 'false',
            'Reg/Force3DoF': 'true',

            # scanを使うならGrid/Sensorは0
            'Grid/Sensor':'0',
